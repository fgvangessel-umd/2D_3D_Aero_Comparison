...
CFDSolver.addSlices("z", np.linspace(0.1, 2.5, 10))
# rst adflow (end)
# ======================================================================
#         Set up flow conditions with AeroProblem
# ======================================================================
# rst aeroproblem (beg)
# ap = AeroProblem(name="wing", alpha=1.5, mach=0.8, altitude=10000, areaRef=2.50, chordRef=1.00, evalFuncs=["cl", "cd"])
# opt:
# alpha = 1.86231935 
# area_con = 1.00
# gen:
...
ap.addDV("alpha", value=alpha, lower=0, upper=10.0, scale=0.1)
# rst aeroproblem (end)
# ======================================================================
...
DVGeo.addLocalDV("local", lower=-0.025, upper=0.025, axis="y", scale=1)
...
# Volume constraints
leList = [[0.01, 0, 0.001], [0.01, 0, 2.49]]
teList = [[0.98, 0, 0.001], [0.97, 0, 2.49]]
DVCon.addVolumeConstraint(leList, teList, nSpan=20, nChord=20, lower=area_con, scaled=True)
...
def objCon(funcs, printOK):
    ...
    funcs["cl_con_" + ap.name] = funcs[ap["cl"]] - cl_con
    if printOK:
